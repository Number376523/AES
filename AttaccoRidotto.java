...
class AttaccoRidotto {
	
	public static final int NUMERO = 2000;
	public static final int COLONNA = 0;                      	 //colonna scelta per il sottospazio D
	public static int[] INSIEME = {1,1,1,1};   					 //insieme I scelto per il sottospazio M
	...
	public static void creaSerie() {
		...
		for (i=0; i<NUMERO; i++) {
			testoIniziale[0][COLONNA] = ((int)(Math.random()*1321))%256; 
			testoIniziale[1][(COLONNA+1)%4] = ((int)(Math.random()*1321))%256;
			testoIniziale[2][(COLONNA+2)%4] = ((int)(Math.random()*1321))%256;
			testoIniziale[3][(COLONNA+3)%4] = ((int)(Math.random()*1321))%256;
			for (j=0; j<4; j++) {
				for (k=0; k<4; k++) {
	...
	public static boolean controlloCollisioni() {
		...
		for (i=0; i<NUMERO; i++) {
			for (j=i+1; j<NUMERO; j++) {
				for (m=0; m<4; m++) {
					for (n=0; n<4; n++) {
						temp0[n][m] = serieCifrata[i][m*4+n] ^ serieCifrata[j][m*4+n];
					}
				}
				if(controlloSpazio(temp0) == true) {
					for (m=0; m<4; m++) {
						for (n=0; n<4; n++) {
							temp1[n][m] = serieIniziale[i][m*4+n];
							temp2[n][m] = serieIniziale[j][m*4+n];
						}
					}
		...
		for (indice1=0; indice1<3; indice1++) {
			for (indice2=0; indice2<3; indice2++) {
				for (indice3=0; indice3<3; indice3++) {
					for (indice4=0; indice4<3; indice4++) {
						...
						if (flag == false) {
							...
							tempMatrix1 = aes.decryption(tempMatrix1, 1);
							tempMatrix2 = aes.decryption(tempMatrix2, 1);
							for (i=0; i<4; i++) {
	...
	public static void main(String[] args) {
		int i;
		boolean flag, superFlag;
		creaSerie();
		i=0;
		superFlag = false;
		while (superFlag == false && i<4) {
			INSIEME[i]=0;
